...
var jwt    = require('jsonwebtoken'); // used to create, sign, and verify tokens // https://github.com/auth0/node-jsonwebtoken
...
var User = {
    ...
    postFindItemAndReturnToken: function (req, res) {
        ...
        var id = req.body.userlogin;
        var pass = sha(req.body.userpassword);
            ...
            if (err){
                ...
                if (!usr[0]) {
                    ...
                    if (usr[0].userpassword !== pass) {
                        ...
                        //todo add CLAIMS to PAYLOAD HERE
                        const payload = {
                        ...
                        var token = jwt.sign(payload, config.supersecret, {
                            expiresIn: '24h', //config.tokenLifeTime // expires in 1 hours = 1440
                        });
